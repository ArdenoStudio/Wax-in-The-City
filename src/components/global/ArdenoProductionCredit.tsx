...
export interface ArdenoProductionCreditProps {
  color?: string;
}

export function ArdenoProductionCredit({ color = "#fff7f9" }: ArdenoProductionCreditProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full flex flex-col items-center justify-center pt-2 pb-1 bg-transparent pointer-events-auto mt-6"
    >
      <style>{`
        @keyframes shine {
      ...
      >
        ...
        <div className="flex flex-col items-center gap-1.5 text-center text-decoration-none">
          ...
          <span
            className={`shine-text font-serif font-medium tracking-[0.14em] text-sm transition-all duration-700 ${
              isAutoShining ? "auto-shine" : ""
            }`}
          >
            Based in LK
          </span>
